...
async def think(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not query:
        await update.message.reply_text("Please provide a question to think about.")
        return
    
    model = get_model(chat_id)
    try:
        message = await update.message.reply_text("🤔 Начинаю размышлять...")
        text_buffer = ""
        for chunk in chat.thinking(chat_id, query, model):
            if chunk["type"] == "thinking":
                await context.bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=message.message_id,
                    text=f"🤔 Размышление: {chunk['content']}\n\nТекущий ответ: {text_buffer}"
                )
