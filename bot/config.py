...
class AI:
    ...
    def __init__(
        ...
        self.url = url or self.default_url.get(self.provider, self.default_url["openai"])
        self.api_key = api_key
        self.model = model or self.default_model.get(self.provider, self.default_model["openai"])
...
class Config:
    ...
    def __init__(self, filename: str, src: dict) -> None:
        ...
        # AI settings.
        self.ai = AI(
            provider=src.get("ai", {}).get("provider"),
            url=src.get("ai", {}).get("url"),
            api_key=src.get("ai", {}).get("api_key"),
            model=src.get("ai", {}).get("model"),
            image_model=src.get("ai", {}).get("image_model"),
            window=src.get("ai", {}).get("window"),
            prompt=src.get("ai", {}).get("prompt"),
            params=src.get("ai", {}).get("params") or {},
        )
...
class SchemaMigrator:
    ...
    def _migrate_v4(cls, old: dict) -> dict:
        ...
        data["ai"]["provider"] = "openai"  # Default provider for old configs
        return data
